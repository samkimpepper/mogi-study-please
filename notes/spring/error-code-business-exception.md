...
### 실제 사례: 하나의 `42501`을 `P0403`과 `P0405`로 분리

사이드 프로젝트에서는 다음 세 실패가 모두 PostgreSQL `42501`로 전달됐다.
...
프론트는 코드만으로 원인을 구분할 수 없어 DB 에러 메시지의 특정 문장을 읽는 sentinel 파싱에 의존했다. 이 방식은 DB 문구가 바뀌면 타입 검사나 컴파일 오류 없이 조용히 잘못 분류될 수 있다.

그래서 DB 함수가 소유권 실패의 의미를 전용 코드로 전달하도록 바꿨다.
...
classifyDataError
  ↓
DATA_ERROR의 안정적인 애플리케이션 의미
  ↓
상황에 맞는 사용자 문구
```

이 변경은 권한 가드가 허용·거부를 판단하는 조건을 바꾼 것이 아니다. 이미 거부하던 상황의 **전달 코드만 더 구체적으로 분리**했다. 차단 뒤 데이터 행이 그대로 유지되는 테스트를 함께 두어 판정 로직에 회귀가 없음을 확인했다.

> 에러 메시지는 사람에게 설명하는 문장이고, 에러 코드는 계층 사이에서 실패의 의미를 전달하는 계약이다.
